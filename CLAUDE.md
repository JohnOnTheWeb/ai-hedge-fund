...
- **Run `purge_orphans.sh` when "already exists" errors hit on redeploy.** RETAIN-policy resources (Secrets Manager entries, plus OpenSearch/AMP/OSIS/ECR under `-c platformDestroy=true`) survive `cdk destroy` and will block the next `cdk deploy` with a `ResourceAlreadyExists` error. `scripts/purge_orphans.sh --execute` filters on `UsedBy=AIHedge` and cleans them up safely.
- **AgentCore Runtime logs may not reach OpenSearch.** The OTel→OSIS ingest endpoint has repeatedly been observed failing with `HTTPSConnectionPool ... Read timed out` (4-second client deadline). Runs still succeed but Runtime container logs silently disappear. Fallback debug paths: (1) `aws lambda invoke` a tool Lambda directly to verify the Gateway contract; (2) Fargate driver logs in CloudWatch (`/aws/ecs/...`) are unaffected; (3) per-ticker result JSONs in `s3://aihedge-config-<account>/runs/<run_id>/<ticker>.json` always land even when traces don't.
